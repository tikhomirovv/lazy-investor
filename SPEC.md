...
1. **Fetcher** (без LLM) скачивает HTML/текст/ленты.
2. **Extractor** извлекает “чистый текст” + метаданные (title, date, source, url).
3. **LLM Fundamental Analyst** получает:
   - извлечённый текст,
   - тикер/инструмент,
...
- **Stage 0**: только расчёты + Telegram отчёты, без intents.
- **Stage 1 (MVP)**: intents + Telegram Approve/Reject, без исполнения.
- **Stage 2**: исполнение после Approve (малые лимиты).
- **Stage 3**: постепенная автоматизация отдельных действий (например, защитные выходы), но только при строгих политиках.
